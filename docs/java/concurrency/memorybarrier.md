...
## [](#volatile变量规则 "volatile变量规则")volatile变量规则

一个用于引出内存屏障的好例子是`volatile变量规则`。

volatile关键字可参考猴子刚开博客时的文章[volatile关键字的作用、原理](https://monkeysayhi.github.io/2016/11/29/volatile%E5%85%B3%E9%94%AE%E5%AD%97%E7%9A%84%E4%BD%9C%E7%94%A8%E3%80%81%E5%8E%9F%E7%90%86/)。volatile变量规则描述了volatile变量的**偏序**语义；这里从volatile变量规则的角度来讲解，顺便做个复习。
...
+   [面试中单例模式有几种写法？](https://monkeysayhi.github.io/2017/09/27/%E9%9D%A2%E8%AF%95%E4%B8%AD%E5%8D%95%E4%BE%8B%E6%A8%A1%E5%BC%8F%E6%9C%89%E5%87%A0%E7%A7%8D%E5%86%99%E6%B3%95%EF%BC%9F/)：“饱汉 - 变种 3”在DCL的基础上，使用volatile修饰单例，以保证单例的可见性。

复杂的利用volatile变量规则（结合了程序顺序规则、传递性）保证变量本身及周围其他变量的偏序：

+   [源码|并发一枝花之ReentrantLock与AQS（1）：lock、unlock](https://monkeysayhi.github.io/2017/12/05/%E6%BA%90%E7%A0%81|%E5%B9%B6%E5%8F%91%E4%B8%80%E6%9E%9D%E8%8A%B1%E4%B9%8BReentrantLock%E4%B8%8EAQS%EF%BC%881%EF%BC%89%EF%BC%9Alock%E3%80%81unlock/)：exclusiveOwnerThread借助于volatile变量state保证其相对于state的偏序。
+   [源码|并发一枝花之CopyOnWriteArrayList](https://monkeysayhi.github.io/2017/10/24/%E6%BA%90%E7%A0%81|%E5%B9%B6%E5%8F%91%E4%B8%80%E6%9E%9D%E8%8A%B1%E4%B9%8BCopyOnWriteArrayList/)：CopyOnWriteArrayList借助于volatile变量array，对外提供偏序语义。

## [](#可见性与重排序 "可见性与重排序")可见性与重排序
...
![计算机内部的缓存架构](../image/计算机内部的缓存架构.png)
...
#### [](#可见性导致的伪·重排序 "可见性导致的伪·重排序")可见性导致的伪·重排序
...
> 
> ![MESI协议的缓存状态机](../image/MESI协议的缓存状态机.png)
> 
> 状态：
> 
...
#### [](#处理器执行时的乱序优化 "处理器执行时的乱序优化")处理器执行时的乱序优化
...
看起来，似乎将上述重排序（或乱序）导致的问题称为“可见性”问题也未尝不可。然而，这种重排序的危害要远远大于单纯的可见性，因为并不是所有的指令都是简单的读或者写——[面试中单例模式有几种写法？](https://monkeysayhi.github.io/2017/09/27/%E9%9D%A2%E8%AF%95%E4%B8%AD%E5%8D%95%E4%BE%8B%E6%A8%A1%E5%BC%8F%E6%9C%89%E5%87%A0%E7%A7%8D%E5%86%99%E6%B3%95%EF%BC%9F/)与[volatile关键字的作用、原理](https://monkeysayhi.github.io/2016/11/29/volatile%E5%85%B3%E9%94%AE%E5%AD%97%E7%9A%84%E4%BD%9C%E7%94%A8%E3%80%81%E5%8E%9F%E7%90%86/)中都提到了`部分初始化`的例子，这种`不安全发布`就是由于重排序导致的。因此，**将重排序归为“可见性”问题并不合适，只能说重排序会导致可见性问题**。
...
## [](#x86架构的内存屏障 "x86架构的内存屏障")x86架构的内存屏障

x86架构并没有实现全部的内存屏障。

### [](#Store-Barrier "Store Barrier")Store Barrier
...
### [](#Load-Barrier "Load Barrier")Load Barrier
...
### [](#Full-Barrier "Full Barrier")Full Barrier
...
## [](#volatile如何解决内存可见性与处理器重排序问题 "volatile如何解决内存可见性与处理器重排序问题")volatile如何解决内存可见性与处理器重排序问题
...
## [](#JVM对内存屏障作出的其他封装 "JVM对内存屏障作出的其他封装")JVM对内存屏障作出的其他封装
...
初始化安全性为解决部分初始化问题带来了新的思路：如果待发布对象的所有域都是final修饰的，那么可以防止对对象的初始引用被重排序到构造过程完成之前。于是，[面试中单例模式有几种写法？](https://monkeysayhi.github.io/2017/09/27/%E9%9D%A2%E8%AF%95%E4%B8%AD%E5%8D%95%E4%BE%8B%E6%A8%A1%E5%BC%8F%E6%9C%89%E5%87%A0%E7%A7%8D%E5%86%99%E6%B3%95%EF%BC%9F/)中的饱汉变种三还可以扔掉volatile，改为借助final的sfence语义：
...
> 参考:
>    [从Java视角理解系统结构(一)CPU上下文切换](https://monkeysayhi.github.io/2017/12/28/一文解决内存屏障/)
> 
...
> +   [聊聊高并发（三十五）Java内存模型那些事（三）理解内存屏障](https://monkeysayhi.github.io/2017/12/28/%E4%B8%80%E6%96%87%E8%A7%A3%E5%86%B3%E5%86%85%E5%AD%98%E5%B1%8F%E9%9A%9C/%E8%81%8A%E8%81%8A%E9%AB%98%E5%B9%B6%E5%8F%91%EF%BC%88%E4%B8%89%E5%8D%81%E4%BA%94%EF%BC%89Java%E5%86%85%E5%AD%98%E6%A8%A1%E5%9E%8B%E9%82%A3%E4%BA%9B%E4%BA%8B%EF%BC%88%E4%B8%89%EF%BC%89%E7%90%86%E8%A7%A3%E5%86%85%E5%AD%98%E5%B1%8F%E9%9A%9C)